...
router.get('/', async (req, res) => {
  try {
    ...
    const { data: fuelData, error: fuelError } = await query
    
    if (fuelError) throw fuelError

    if (fuelData && fuelData.length > 0) {
      const vehicleIds = [...new Set(fuelData.map(f => f.vehicle_id))]
      ...
      const filteredData = fuelData.filter(f => allowedVehicleIds.includes(f.vehicle_id))
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const { data: fuelExpense, error: fuelError } = await supabase
      .from('fuel_expenses')
      .select('*')
      .eq('id', req.params.id)
      .single()
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { data: fuelExpense, error: findError } = await supabase
      .from('fuel_expenses')
      .select('vehicle_id')
      .eq('id', req.params.id)
      .single()
